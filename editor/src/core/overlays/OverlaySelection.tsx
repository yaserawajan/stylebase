    ...
    return (
        <div key="selection" className="overlay-selection" style={{ top, left, width, height }}>
            <div key="rd" style={{ position: "relative", top: 0, left: 0, width: "100%" }}>
                ...
                { width > 50  && <div key="width" className="width"><IconLA icon="arrow-left"/> {actualRect.width} px <IconLA icon="arrow-right"/></div>}
                { height > 50 && <div key="height" className="height" style={{ width: height }}><IconLA icon="arrow-left"/>{actualRect.height} px <IconLA icon="arrow-right"/></div>}
            </div>
        </div>
    );
}